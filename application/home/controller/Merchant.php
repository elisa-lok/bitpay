 ...
 ['status','交易状态'],
	...
	public function deal() {//放行usdt
		if (request()->isPost()) {
			...
			$sellerParentMoney = $buyerParentMoney = $sellerAgentExist = $buyerAgentExist = 0;
			/*if ($seller['pid']) {
			...
			if ($seller['pid'] > 0 && $seller['reg_type'] == 2) {
				...
				if ($firstParent['agent_check'] == 1) {
					($seller['tariff'] > 0 && $firstParent['tariff'] <= $seller['tariff']) && $this->error('配置异常, 请联系管理员,code:231');
					...
					($seller['tariff'] <= 0 && ($secondParent && $firstParent['tariff'] > $secondParent['tariff']) || ($thirdParent && $secondParent['tariff'] > $thirdParent['tariff'])) && $this->error('配置异常, 请联系管理员, code:232');
					if ($thirdParent) {
						$sellerParentMoney   = $orderInfo['deal_num'] * $thirdParent['tariff']; // 卖家以及代理总费用
						...
						$sellerParentMoney   = $orderInfo['deal_num'] * $secondParent['tariff']; // 卖家以及代理总费用
						$sellerParentMoney_2 = $orderInfo['deal_num'] * ($secondParent['tariff'] - $firstParent['tariff']);
					} else {
						$sellerParentMoney = $orderInfo['deal_num'] * $firstParent['tariff']; // 卖家以及代理总费用
					}
			...
			$mum           = $orderInfo['deal_num'] - $totalFee;
			//实际到账金额
			...
			try {
				...
				//卖家代理利润
				if ($sellerParentMoney_1 > 0 && $sellerAgentExist) {
					// 一级
					$rsArr = agentReward($firstParent['id'], $orderInfo['sell_id'], $sellerParentMoney_1, 3, $id);
					!$rsArr[0] && $this->rollbackAndMsg('订单操作失败,错误码:10005', $id);
					!$rsArr[0] && $this->rollbackAndMsg('订单操作失败,错误码:10006', $id);
					// 二级
					if ($sellerParentMoney_2 > 0) {
						$rsArr = agentReward($secondParent['id'], $orderInfo['sell_id'], $sellerParentMoney_2, 3, $id);
