...
class Merchant extends Base{
    ...
    public function editad(){
        ...
        if(request()->isPost()){
            ...
            $ad_no = $this->getadvno();
            $flag = $model2->updateOne(['id'=>$id,'min_limit'=>$min_limit,'max_limit'=>$max_limit,'state'=>1,'pay_method'=>$_POST['bank'],'pay_method2'=>$_POST['zfb'],'pay_method3'=>$_POST['wx'],'pay_method4'=>$_POST['ysf'],'amount'=>$amount,'price'=>$price,'state'=>1]);
            if($flag['code'] == 1){
				$count = $model2->where('userid', session('uid'))->where('state', 1)->where('amount', 'gt', 0)->count();
                $model->updateOne(['id'=>session('uid'), 'ad_on_sell'=>$count]);
