...
class MerchantModel extends Model {
	...
	public function insertOne($param) {
		try {
			...
			return FALSE === $result ? ['code' => -1, 'data' => '', 'msg' => $this->getError()] : ['code' => 1, 'data' => '', 'msg' => '恭喜你，注册成功'];
	...
	public function updateOne($param) {
		try {
			...
			return FALSE === $result ? ['code' => 0, 'data' => '', 'msg' => $this->getError()] : ['code' => 1, 'data' => '', 'msg' => '信息修改成功'];
		} catch (\PDOException $e) {
	...
	public function login($username, $password) {
		...
		if (empty($find)) return ['code' => -1, 'msg' => '用户不存在'];
		if ($find['reg_check'] == 0) return ['code' => -1, 'msg' => '用户待审核'];
		if ($find['reg_check'] == 2) return ['code' => -1, 'msg' => '用户未审核通过'];
		if ($find['password'] != md5($password)) return ['code' => -1, 'msg' => '登陆密码错误'];
	...
	public function getApiLog($where, $order) {
		$join = [['__MERCHANT__ b', 'a.duid=b.id', 'LEFT'],];
	...
	public function getTraderReward($where, $order) {
		$join = [['__ORDER_BUY__ b', 'a.orderid=b.id', 'LEFT'],];
