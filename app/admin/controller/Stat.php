...
class Stat extends Base {
	...
	public function run($d = 1) {
		$d    = $d < 1 ? 30 : (int)$d;
		$time = time() - 86400;
		$db   = Db::name('stat_daily');
		for ($i = $d; $i > 0; $i--) {
			$isExist = $db->where('id', date('Ymd', $time))->count(1);
			!$isExist && $db->insert($this->doStat($time));
			$time -= 86400;
		}
		die('All Done!!');
	}
	...
	private function doStat(int $time) {
		...
		//代理商奖励总和
		$profit['agent_reward'] = getTotalInfo(['create_time' => $timeSql, 'uid' => ['gt', 0]], 'agent_reward', 'amount');
		//承兑商奖励总和
		$profit['user_reward'] = getTotalInfo(['create_time' => $timeSql], 'trader_reward', 'amount');
		// 成功率统计
		$profit['succ_rate'] = $profit['order_count'] = 0;
		...
		if ($statusArr) {
			...
			$profit['order_count'] = array_sum($statusArr);
		}
		return $profit;
	}
}