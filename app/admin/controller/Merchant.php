...
class Merchant extends Base {
  ...
  public function stat_platform_clean() {
    !request()->isAjax() && !request()->isPost() && showMsg('页面不存在');
    $args = input('post.');
    ...
    Db::name('merchant_user_withdraw')->where($sql)->update(['fee' => 0]);
    Db::name('merchant_withdraw')->where($sql)->update(['fee' => 0]);
    $sql = ['status' => 4, 'ctime' => ['between', [$stime, $etime]]];
