...
class Tx {
	...
	public function getNotify($id) {
		...
		!$seller && showMsg('用户不存在');
		$orderInfo = Db::name('order_buy')->where('sell_id', $seller['id'])->whereIn('status', '0,1')->find();
		!$orderInfo && showMsg('订单不存在');
		empty($args['money']) && showMsg('金额错误');
		($args['money'] != $orderInfo['deal_amount']) && showMsg('金额不匹配');
		// 开始处理订单
		$mchModel = new MerchantModel();
		empty($orderInfo) && showMsg('订单不存在');
		($orderInfo['status'] == 5) && showMsg('订单已经被取消');
		($orderInfo['status'] == 6) && showMsg('订单申诉中，无法释放');
		//20190830修改,不打款,也可以确认
		$unpay = ($orderInfo['status'] == 0) ? 1 : 0;
		($orderInfo['status'] >= 3) && showMsg('此订单已处理,无需再次处理');
		$buyer = $mchModel->where('id', $orderInfo['buy_id'])->find();
		($seller['usdtd'] < $orderInfo['deal_num']) && showMsg('您的冻结不足，交易失败');
		// 锁定操作 代码执行完成前不可继续操作 60秒后可再次点击操作
		Cache::has($id) && showMsg('操作频繁,请稍后重试');
		$lock = Cache::set($id, TRUE, 60);
		!$lock && showMsg('锁定操作失败，请重试。');
		//盘口费率
		...
		if ($seller['pid'] > 0) {
			...
			$sellerFirstMoney < 0 && showMsg('配置异常, 请联系管理员,错误码:231');
			// 二级
			if ($firstParent && $firstParent['pid'] > 0) {
				...
				$sellerSecondMoney < 0 && showMsg('配置异常, 请联系管理员,错误码:232');
			}
			// 三级
			if ($secondParent && $secondParent['pid']) {
				...
				$sellerThirdMoney < 0 && showMsg('配置异常, 请联系管理员,错误码:233');
			}
		...
		if ($buyer['pid']) {
			...
			if ($buyerParent['agent_check'] == 1 && $buyerParentGet > 0) {
				...
				($sellerParentMoney + $buyerParentMoney + $sellerAwardMoney > $totalFee) && showMsg('配置异常, 请联系管理员,错误码:234');
			}
		}
		...
		$platformMoney < 0 && showMsg('配置异常, 请联系管理员,错误码:235');
		Db::startTrans();
		try {
			// 订单更新
			...
			!(Db::name('order_buy')->where('id', $id)->update($updateCondition)) && $this->rollbackShowMsg('订单更新失败', $id);
			// 卖家减去冻结
			!balanceChange(FALSE, $orderInfo['sell_id'], 0, 0, -$orderInfo['deal_num'], 0, BAL_SOLD, $orderInfo['id']) && $this->rollbackShowMsg('冻结余额不足,错误码:10001', $id);
			// 卖家增加数据, 增加平均交易时间:秒average , 以及交易单数transact
			!$mchModel->where('id', $orderInfo['sell_id'])->update(['transact' => Db::raw('transact+1')]) && $this->rollbackShowMsg('卖家信息操作失败,错误码:10002', $id);
			// 卖家卖单奖励
			if ($sellerAwardMoney > 0) {
				!balanceChange(FALSE, $orderInfo['sell_id'], $sellerAwardMoney, 0, 0, 0, BAL_COMMISSION, $orderInfo['id']) && $this->rollbackShowMsg('订单操作失败,,错误码:10003', $id);
				!(Db::name('trader_reward')->insert(['uid' => $orderInfo['sell_id'], 'orderid' => $orderInfo['id'], 'amount' => $sellerAwardMoney, 'type' => 0, 'create_time' => time()])) && $this->rollbackShowMsg('订单操作失败,错误码:10004', $id);
			}
			//卖家代理利润
			if ($sellerFirstMoney > 0) {
				// 一级
				$rsArr = agentReward($firstParent['id'], $orderInfo['sell_id'], $sellerFirstMoney, 3, $id);
				!$rsArr[0] && $this->rollbackShowMsg('订单操作失败,错误码:10005', $id);
				!$rsArr[1] && $this->rollbackShowMsg('订单操作失败,错误码:10006', $id);
				// 二级
				if ($sellerSecondMoney > 0) {
					$rsArr = agentReward($secondParent['id'], $orderInfo['sell_id'], $sellerSecondMoney, 3, $id);
					!$rsArr[0] && $this->rollbackShowMsg('订单操作失败,错误码:10007', $id);
					!$rsArr[1] && $this->rollbackShowMsg('订单操作失败,错误码:10008', $id);
				}
				// 三级
				if ($sellerThirdMoney > 0) {
					$rsArr = agentReward($thirdParent['id'], $orderInfo['sell_id'], $sellerThirdMoney, 3, $id);
					!$rsArr[0] && $this->rollbackShowMsg('订单操作失败,错误码:10009', $id);
					!$rsArr[1] && $this->rollbackShowMsg('订单操作失败,错误码:10010', $id);
				}
			}
			// 买家加币
			!balanceChange(FALSE, $orderInfo['buy_id'], $sum, 0, 0, 0, BAL_BOUGHT, $orderInfo['id']) && $this->rollbackShowMsg('订单操作失败,错误码:10011', $id);
			!$mchModel->where('id', $orderInfo['buy_id'])->update(['transact' => Db::raw('transact+1')]) && $this->rollbackShowMsg('订单操作失败,错误码:10012', $id);
			// 买家代理
			if ($buyerParentMoney > 0 && $buyerAgentExist) {
				$rsArr = agentReward($buyer['pid'], $orderInfo['buy_id'], $buyerParentMoney, 4, $id);//4
				!$rsArr[0] && $this->rollbackShowMsg('订单操作失败,错误码:10013', $id);
				!$rsArr[1] && $this->rollbackShowMsg('订单操作失败,错误码:10014', $id);
			}
			// 平台利润
			if ($platformMoney > 0) {
				$rsArr = agentReward(-1, 0, $platformMoney, 5, $id);//5
				!$rsArr[1] && $this->rollbackShowMsg('订单操作失败,错误码:10015', $id);
			}
			// 提交事务
			Db::commit();
			...
			$this->rollbackShowMsg('交易失败，参考信息：' . $e->getMessage(), $id);
		}
	}
	...
	private function logNotify() {
		...
		$notifyPath = 'get_notify_' . date('ymd') . '.log';
		// 异步回调通知形式
		if ($_POST) {
	...
	private function rollbackShowMsg($msg) {
		Db::rollback();
		showMsg($msg, 0);
	}
}