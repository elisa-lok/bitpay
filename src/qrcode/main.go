...
func createImg(ct string) (string, string, string, error) {
	q := &QRCODE{size: 300, ext: "PNG", content: ct}
	if strings.Contains(ct, "qr.alipay.com") {
		q.icon = "ALIPAY"
		q.background = "ALIPAY"
	} else if strings.Contains(ct, "wxp://") || strings.Contains(ct, "w.url.cn") || strings.Contains(ct, "wx.tenpay.com") {
		q.icon = "WXPAY"
		q.background = "WXPAY"
	} else if strings.Contains(ct, "qr.95516.com") {
		q.icon = "UNION"
		q.background = "UNION"
	} else {
		fmt.Println("cannot support qrcode" + ct)
		return "", "", "", errors.New("cannot support qrcode")
	}
	return createQrcode(q)
}
...
func parse(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json;charset=utf-8")
	isGenQR := r.Form.Get("qr")
	if len(isGenQR) > 0 {
		resp.Data.ImgBase, resp.Data.ImgBaseIcon, resp.Data.ImgBaseBg, err = createImg(resp.Data.ImgContent)
		if err != nil {
			w.WriteHeader(500)
			w.Write([]byte(err.Error()))
		}
...
func delFile(filePath string)  {
	...
	if err != nil {
		fmt.Println("file stat : " +err.Error())
		return
	}
	err = os.Remove(filePath) // 异步删除文件
	if err != nil {
		fmt.Println("del file : " + err.Error())
		return
	}
	fmt.Println("del file success : " + filePath)
}
